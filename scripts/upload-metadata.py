...
"""
import os, sys, json, time, base64, mimetypes, re, subprocess
import urllib.request, urllib.error
from pathlib import Path

def tg_ios(html):
    try:
        subprocess.run(["tg", "app_ios", html], timeout=5, capture_output=True)
    except Exception:
        pass
...
def process_app(creds, slug, app_data):
    ...
    if version_loc_id:
        ...
        sc_ok, sc_fail = 0, 0
        if sc_dir.exists():
            # iPhone 6.7" screenshots (APP_IPHONE_67, exclude ipad- and iphone65- prefixes)
            iphone_files = sorted(f for f in sc_dir.glob("*.png")
                                  if not f.name.startswith("ipad-") and not f.name.startswith("iphone65-"))[:3]
            # iPhone 6.5" screenshots (APP_IPHONE_65, iphone65- prefix → 1284×2778)
            iphone65_files = sorted(sc_dir.glob("iphone65-*.png"))[:3]
            # iPad screenshots (ipad- prefixed, generated by generate-ipad-screenshots.py)
            ipad_files = sorted(sc_dir.glob("ipad-*.png"))
            all_files = iphone_files + iphone65_files + ipad_files
            if all_files:
    ...
    app_name = " ".join(w.capitalize() for w in slug.replace("-", " ").split())
    sc_types = []
    if iphone_files: sc_types.append(f"{len(iphone_files)}×6.7\"")
    if iphone65_files: sc_types.append(f"{len(iphone65_files)}×6.5\"")
    if ipad_files: sc_types.append(f"{len(ipad_files)}×iPad")
    sc_summary = " | ".join(sc_types) if sc_types else "no screenshots"
    status_icon = "✅" if sc_fail == 0 else "⚠️"
    tg_ios(
        f"{status_icon} <b>{app_name}</b>\n"
        f"📋 Metadata: title ✓ | subtitle ✓ | description ✓ | keywords ✓\n"
        f"🖼 Screenshots: {sc_summary} ({sc_ok} uploaded{f', {sc_fail} failed' if sc_fail else ''})\n"
        f"📱 App ID: {app_id}"
    )
